...
class GraphPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        shankfile = csv.reader(open('shanks.csv'))

        inhib_time = tk.Label(self, text="")
        inhib_time.pack({"side": "top"})

        shankval = tk.Label(self, text="")
        shankval.pack({"side": "right"})

        start_button = tk.Button(self, text="Start",
                                 command=lambda: controller.start_graphing(file, inhibfile, inhib_time, shankfile, shankval))
        ...
        yes_button = tk.Button(self, text="Yes",
                                command=lambda: controller.do_graphing(file, inhibfile, inhib_time, shankfile, shankval, 1))
        yes_button.pack({"side":"left"})

        no_button = tk.Button(self, text="No",
                              command=lambda: controller.do_graphing(file, inhibfile, inhib_time, shankfile, shankval, 0))
